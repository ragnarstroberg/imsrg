...
run = 1
for i in range(run):
    e,vs,lvs=arnoldi_proc(htc_vs, Norm_vs_new2, Norm4, Hs, chi, 100,2,ms,tdm_op)
## restart the arnorldi
    chi=chi*0.
    
    for k in range(len(e)):
        for ks in range(1):
            chi = chi + lvs[k]*vs[k,ks]
    chi = chi / Norm_vs_new2(chi,chi,tdm_op)



#e,v1,v2=lanczos_proc(htc_vs, Norm_vs_new, Hs, chi, 160, 2,ms)
#e,vs,v2=arnoldi_proc(htc_vs, Norm_vs_new3, Hs, chi, 180,2,ms,tdm_op)


# runs=1
# ard_vectors=[]
# for ard_step in range(runs):
#     unt.SetRandomSeed(ard_step*100)
#     h3= unt.RandomOp( ms, rank_j,  rank_Tz, parity, particle_rank,herm)
#     chi= gm.GetVSEOM_ladder(h3,0)
#     nm=Norm_vs_new2(chi,chi,tdm_op)
#     chi=chi/np.sqrt(nm)
