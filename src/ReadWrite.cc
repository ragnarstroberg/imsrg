...
void ReadWrite::WriteTwoBody_Binary_myg(std::string filename, Operator& Hbare)
{
  ...
  std::vector<fortran_int> ib;
  std::vector<fortran_int> ic;
  std::vector<fortran_int> id;
  ...
  ia.reserve(total_nelts);
  ib.reserve(total_nelts);
  ic.reserve(total_nelts);
  id.reserve(total_nelts);
  jj.reserve(total_nelts);
  v2.reserve(total_nelts);
  ...
  if (cnt > 0)
  {
    outfile.write(reinterpret_cast<const char*>(ia.data()),
                  static_cast<std::streamsize>(cnt) * sizeof(fortran_int));
    outfile.write(reinterpret_cast<const char*>(ib.data()),
                  static_cast<std::streamsize>(cnt) * sizeof(fortran_int));
    outfile.write(reinterpret_cast<const char*>(ic.data()),
                  static_cast<std::streamsize>(cnt) * sizeof(fortran_int));
    outfile.write(reinterpret_cast<const char*>(id.data()),
                  static_cast<std::streamsize>(cnt) * sizeof(fortran_int));
    outfile.write(reinterpret_cast<const char*>(jj.data()),
                  static_cast<std::streamsize>(cnt) * sizeof(fortran_int));
    outfile.write(reinterpret_cast<const char*>(v2.data()),
                  static_cast<std::streamsize>(cnt) * sizeof(fortran_real8));
  }
...
void ReadWrite::ReadTwoBody_Binary_myg(std::string filename, Operator& op)
{
  ...
  {
    std::cerr << "************************************" << std::endl
              << "** Trouble reading binary file !!! ** " << filename << std::endl
              << "************************************" << std::endl;
    return;
  }
  ...
  if (cnt > 0)
  {
    ...
    infile.read(reinterpret_cast<char*>(ib.data()), sizeof(fortran_int) * cnt);
    infile.read(reinterpret_cast<char*>(ic.data()), sizeof(fortran_int) * cnt);
    infile.read(reinterpret_cast<char*>(id.data()), sizeof(fortran_int) * cnt);
    infile.read(reinterpret_cast<char*>(jj.data()), sizeof(fortran_int) * cnt);
