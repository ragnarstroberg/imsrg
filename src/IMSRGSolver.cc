...
void IMSRGSolver::Solve_flow_RK4()
{
  ...
  for (istep = 1; s < smax; ++istep)
  {
    ...
    Operator Factorized_3B( *modelspace, 0,0,0,2);
    ...
    for (int i = 0; i < nops; i++)
    {
      ...
      if (Commutator::use_factorized_correction)
      {
        Factorized_3B.Erase();
        Commutator::comm223_231_Factorization(Eta, FlowingOps[i], Factorized_3B);
        Commutator::comm223_232_Factorization(Eta, FlowingOps[i], Factorized_3B);
        K2[i] += 0.5 * ds * Factorized_3B;
      }
    ...
    for (int i = 0; i < nops; i++)
    {
      ...
      if (Commutator::use_factorized_correction)
      {
        ...
        Commutator::comm223_231_Factorization(Eta, FlowingOps[i], Factorized_3B);
        Commutator::comm223_232_Factorization(Eta, FlowingOps[i], Factorized_3B);
        K3[i] += 0.5 * ds * Factorized_3B;
      }
      Ktmp[i] = FlowingOps[i] + 1. * ds * K3[i];
    }
