...
class UnitTest
{
 ...
 public:
  ...
  DaggerOperator RandomDaggerOp( ModelSpace& modelspace, index_t Q);
  ...
  double GetMschemeMatrixElement_1leg( const DaggerOperator& Op, int a, int ma );
  double GetMschemeMatrixElement_3leg( const DaggerOperator& Op, int a, int ma, int b, int mb, int c, int mc );
...
//  void TestCommutators3();
  ...
  bool Test_comm211sd(        const Operator& X, const DaggerOperator& Y   );
  bool Test_comm231sd(        const Operator& X, const DaggerOperator& Y   );
  bool Test_comm431sd(        const Operator& X, const DaggerOperator& Y   );
  bool Test_comm233sd(        const Operator& X, const DaggerOperator& Y   );
  bool Test_comm413sd(        const Operator& X, const DaggerOperator& Y   );
  bool Test_comm433_pp_hh_sd( const Operator& X, const DaggerOperator& Y   );
  bool Test_comm433sd_ph(     const Operator& X, const DaggerOperator& Y   );
