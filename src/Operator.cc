...
Operator Operator::DoNormalOrdering()
{
   ...
   for (int ch=0;ch<nChannels;++ch)
   {
      ...
      // Zero body part
      for (int& k : tbc.KetIndex_hh) // loop over hole-hole kets in this channel
      {
        opNO.ZeroBody += TwoBody[ch](k,k) * (2*J+1);  // <ab|V|ab>  (a,b in core)
      }
