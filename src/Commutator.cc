...
namespace {
  ...
  class Basis3BLookupSingleChannel {
    public:
      Basis3BLookupSingleChannel(std::size_t i_ch_3b, std::size_t i_ch_2b, const Operator& Y);
      ...
      std::size_t Get2BStateP(std::size_t state_2b_i) const { return states_2b_p[state_2b_i]; }
      std::size_t Get2BStateQ(std::size_t state_2b_i) const { return states_2b_q[state_2b_i]; }

      std::size_t GetNum3BStates() const { return states_3b_pqr.size(); }
      std::size_t Get3BStateP(std::size_t state_3b_i) const { return states_3b_p[state_3b_i]; }
      std::size_t Get3BStateQ(std::size_t state_3b_i) const { return states_3b_q[state_3b_i]; }
      std::size_t Get3BStateR(std::size_t state_3b_i) const { return states_3b_r[state_3b_i]; }

      const std::vector<std::size_t>& GetRecouplingIndices(std::size_t state_3b_i) const {
        return states_3b_indices[state_3b_i];
      }
      const std::vector<double>& GetRecouplingFactors(std::size_t state_3b_i) const {
        return states_3b_recoupling[state_3b_i];
      }

      const std::vector<std::size_t> Get3rdStateBasis() const { return basis_r; }

      std::size_t GetStateLocalIndex(std::size_t p, std::size_t q, std::size_t r) const {
        return pqr_lookup[p * wrap_factor * wrap_factor + q * wrap_factor + r];
      }

      int IsStateValid(std::size_t p, std::size_t q, std::size_t r) const {
        return pqr_lookup_validity[p * wrap_factor * wrap_factor + q * wrap_factor + r];
      }
    private:
...
int GetPhase(std::size_t orb_i, std::size_t orb_j, int J_ij, const Operator& Y) {
  int jj_i = Y.modelspace->GetOrbit(orb_i).j2;
  int jj_j = Y.modelspace->GetOrbit(orb_j).j2;
  return Y.modelspace->phase((jj_i + jj_j) / 2 - J_ij);
}

double GetSPStateOcc(std::size_t orb_i, const Operator& Y) {
...
void comm232ss_expand( const Operator& X, const Operator& Y, Operator& Z )
{
  ...
  for (std::size_t i_ch_3b = 0; i_ch_3b < Y.modelspace->GetNumberThreeBodyChannels(); i_ch_3b += 1) {
    ...
    const std::vector<std::size_t> chans_2b = Get2BChannelsValidIn3BChannel(jj1max, ch_3b, Y);

    std::vector<Basis3BLookupSingleChannel> lookup_vec;
