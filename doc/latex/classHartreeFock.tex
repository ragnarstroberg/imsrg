...
Construct an unnormalized three-\/body monopole interaction \[ \langle iab | \bar{V}^{(3)} | jcd \rangle = \sum\limits_{J,J_{12}}\sum_{Tt_{12}}(2J+1)(2T+1) \langle (ia)J_{12}t_{12};b JT| V^{(3)} | (jc)J_{12}t_{12}; d JT\rangle \]-\/body density matrix $ <i|\rho|j> = \sum\limits_{\beta} n_{\beta} <i|\beta> <\beta|j> $ where $n_{\beta} $ ensureas that beta runs over H\-F orbits in the core (i.\-e. below the fermi surface) \hypertarget{classHartreeFock_a84fe0eb16f6e5835c920bf8fa98c4442}{\index{Hartree\-Fock@{Hartree\-Fock}!Update\-F@{Update\-F}}
