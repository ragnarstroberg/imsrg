...
for ((emax=12;emax<=12;emax+=2)); do

flowfile=$HOME/ragnar_imsrg/output/BCH_SingleRefsat400_hw${hw}_O16_${method}_e${emax}_s${smax}.dat
...
${exe} 2bme=${vnn} 3bme=${v3n} emax=${emax} e3max=${e3max} nucleus=${nucl} hw=${hw} flowfile=${flowfile} smax=${smax} ${file3} method=${method}
END
